...
<script>
	...
	var page = 'bd';
	var elementType ='Brand';
	var cardUrl="./BrandDashboard";
	genrefilter='<%=request.getParameter("genre")%>';
	channelfilter="<%=request.getParameter("channel")%>";
	brandfilter="<%=request.getParameter("brandfilter")%>";
 ...
 -->	<li><a href="./Logout"><i
			...
			<div class="dashbar">
				...
				<div class="col-sm-4">
					<div class="Bar-check">
						<ul>
							<li style="">&nbsp;<input onclick="allOnloadBrandGraph(page);" id="time"
								value="Playout" type="radio" checked="checked" name="options">&nbsp;Playout&nbsp;
							</li>

							<li>&nbsp;<input type="radio" onclick="allOnloadBrandGraph(page)"
...
.open>.dropdown-menu {
	...
	width: 175px;
						...
						<div class="form-group">
							...
							<!-- for below Select -- onchange='allOnloadBrandGraph(page);  -->
												<div class="form-group">
...
<div class="form-group">
								...
								<div class="dropdown cq-dropdown" data-name='statuses'>
									...
									<ul class="dropdown-menu selectable"
										aria-labelledby="dropdown1" id="brand">
									<input type="text" class="search-filter" onkeyup="searchFilter($(this))" ref="brand" placeholder="Search Brand"/>
										<div class="searchList">
      ...
      <c:choose>
            <c:when test="${fn:contains(brand, ' ')}">
            <c:set var="brandname" value="${fn:replace(fn:trim(brand), ' ','_')}" />
            
            </c:when>
            <c:otherwise>
            <c:set var="brandname" value="${brand}" />
            </c:otherwise>
            </c:choose>     
            ...
            <c:choose>
            <c:when test="${fn:contains(brandname, '.')}">
            <c:set var="brandname2" value="${fn:replace(fn:trim(brandname), '.','_')}" />  
            
            </c:when>
            <c:otherwise>
            <c:set var="brandname2" value="${brandname}" />
            </c:otherwise>
            </c:choose>                              
											<li data-toggle='tooltip' title="${fn:trim(brand)}" class="ui-widget-content brand" id="option_${fn:replace(fn:trim(brand), ' ','_');fn:replace(fn:trim(brandname2),search, replace)}">${fn:trim(brand)}</li>
										</c:forEach>
										</div>
									</ul>
								</div>
						...
						<div class="col-sm-3" >
							<div
								...
								<h4
									style="color: rgb(255, 255, 255, 0.8); font-size: 21px; font-weight: 100 !important;">Top Brands</h4>
							</div>
							...
							<div
								...
								<div style="text-align: center; color: rgb(85, 206, 249);">
										...
										<span style="font-size: 34px;" id="BrandCount"></span>
								</div>
								<h4
									style="text-align: center; color: rgb(255, 255, 255, 0.8); font-size: 21px;">Brands</h4>
										...
										<div class="col-sm-4" style="display: block;">
											...
											<div class="card card-chart">
												<div class="card-header">
													<h4 class="card-title">
														<i class="tim-icons icon-tv-2"></i><a href="./BrandDashboard" style="color: black;">Top Brands</a>
													</h4>
												...
												<div class="card-body" id="topChannelChartBody">
													<div class="chart-area" id="piegraph"
														...
														<div id="chartLinePurple" class="topbrands"></div>
												...
												<div class="card-header">
													<h4 class="card-title">
														<i class="tim-icons icon-watch-time"></i>Hourly Playout
													 	...
													 	<select id="hourlyunits" class="hoursUnitSelect" style="background: #5d6369;float: right;border:none;outline:none;"  onchange="hourlyPlayoutData('/getHourlyPlayoutByBrand','rgb(85, 206, 249)', 'rgb(85, 206, 249)', '0.4','hourlyPlayoutLineChart');"> 
...
if(brandfilter=='null'){
									...
									if (brandfilter.includes('~')) {
									    array = brandfilter.split("~");
										for(var i=0;i<array.length;i++){
											var brandids=array[i].replace(/[^A-Z0-9]/ig, "_");
										    $('#option_'+brandids).selectable();							
				                            $('#option_'+brandids).removeClass();
				                            $('#option_'+brandids).addClass("ui-widget-content brand ui-selectee ui-selected ui-selected-multi");
			 	                            handleTags(array[i],'option_'+brandids, "brand", "add");
			 	                    	}
									}
									else{
										array = brandfilter;	
										var brandids=array.replace(/[^A-Z0-9]/ig, "_");
									    $('#option_'+brandids).selectable();							
			                            $('#option_'+brandids).removeClass();
			                            $('#option_'+brandids).addClass("ui-widget-content brand ui-selectee ui-selected ui-selected-multi");
			                            handleTags(array,'option_'+brandids, "brand", "add");
			...
			$("#brand li").on("click",function(e) {
											if ($(this).hasClass("ui-selected-multi")) {
												$(this).removeClass(
																"ui-selected ui-selected-multi");
												handleTags($(this).text(), $(this).attr('id'), "brand",
														"remove");
											} else {
												$(this).addClass(" ui-selected-multi");
												handleTags($(this).text(), $(this).attr('id'), "brand",
