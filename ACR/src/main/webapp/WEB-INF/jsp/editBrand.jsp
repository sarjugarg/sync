...
<body>
		...
		<i class="fa fa-external-link-square"></i> Edit Brand Image Details
...
<form  class="form-horizontal" action="${context}/editProduct"  role="form" modelAttribute="Product" method='POST' id="div1">
				...
				<div class="form-group">
					...
					<div class="col-xs-6  col-sm-3">
						<input type="text" class="form-control" name="BrandName" id="BrandName" value="${list.brandName }" readonly="true">
					</div>
					</div>
				...
				<%-- <div class="form-group">
							...
							<div class="col-xs-6 col-sm-3" id="dp1">
								<select class="form-control " name="productId" id="productId"
									required>
									 ...
									 <c:choose>
									<c:when test="${map.key==list.brandId}"> 
									<option value="${ list.brandId}">${ map.value}</option>
									 </c:when>
									 </c:choose>
									</c:forEach> 
									<c:forEach items="${map}" var="map">
					...
					<div class="form-group">
					<input type="text" name="productId" id="productId" value="${list.brandId }" style="display:none">
					</div>
					<div class="form-group">
					...
					<div class="col-xs-6  col-sm-3" style="display: inline-flex;">
						...
						<img src="${imageurl}${list.filename}"
...
function create1(){
	var brandid = document.getElementById('productId').value;
	//var brandImageId = document.getElementById('productImageId').value;BrandName
	var BrandName = document.getElementById('BrandName').value;
		...
		$.ajax({
			        	url: '${context}/editbrand/'+brandid+'/'+'?BrandName='+BrandName,
			            data :formData,
			            processData : false,
			            contentType : false,
			            ...
			            success : function(data) {
			            	...
			            	window.location.href="${context}/brandManagement";
				        		...
				        		document.getElementById("spantext").innerHTML = "Brand is already Exist,please insert Fresh Brand. ";
				        	}
			            },
			            error : function() {
			            	alert("error");
...
function deleteLogo(){
	 ...
	 var imageid=document.getElementById('productImageId').value;
	// alert("brand ImageId id="+imageid);	
		
		
	 $.ajax({
	url: '${context}/deletebrandlogo/'+imageid,
	data :formData,
	processData : false,
	contentType : false,
	type : 'POST',
...
function ConfirmDelete()
{
	var x = confirm("Are you sure you want to delete?");
	  if (x){
	 var imageid=document.getElementById('productId').value;
		// alert("brand ImageId id="+imageid);	
