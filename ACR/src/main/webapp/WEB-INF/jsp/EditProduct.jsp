...
<body>
		...
		<i class="fa fa-external-link-square"></i> Edit Product Image Details
...
<form  class="form-horizontal" action="${context}/editProduct"   role="form" modelAttribute="Product" method='POST' id="div1">
					...
					<div class="col-xs-6  col-sm-3">
						<input type="text" class="form-control" name="productname" id="productname" value="${list.productName}" readonly="true">
					</div>
					</div>
							...
							<div class="col-xs-6 col-sm-3" id="dp1">
								<select class="form-control " name="productId" id="productId"
									required>
									 ...
									 <c:choose>
									<c:when test="${map.key==list.productId}"> 
									<option value="${ list.productId}">${ map.value}</option>
									 </c:when>
									 </c:choose>
									</c:forEach> 
									<c:forEach items="${map}" var="map">
					...
					<div class="form-group">
					<input type="text" name="productId" id="productId" value="${list.productId }" style="display:none">
					</div>
					<div class="form-group">
					...
					<div class="col-xs-6  col-sm-3" style="display: inline-flex;">
						...
						<img src="${productImageUrl}${list.filename}"
...
function create1(){
	...
	//var b = document.getElementById('productImageId').value;
	...
	//	formData.append('data', JSON.stringify(resultArr)); 
		$.ajax({
			        	//url: '${context}/editProduct/'+a+'/'+b+'/'+productname,
			        	url: '${context}/editProduct/'+a+'/'+'?productname='+productname,
			            data :formData,
			            processData : false,
			            contentType : false,
			            type : 'POST',
			            ...
			            {
			            	if(data=="Updated"){
			            	window.location.href="${context}/productmanagement";
			            }
			        	else{
			        		
			        		document.getElementById("spantext").innerHTML = "Product is already Exist,please insert Fresh Product. ";
			        	}
			            },
			            error : function() {
			            	//alert("error");
...
function deleteLogo(){
	...
	// //alert("productImageId id="+productImageId);	
		
		
	 $.ajax({
	url: '${context}/deleteproductlogo/'+productId,
	data :formData,
	processData : false,
	contentType : false,
	type : 'POST',
