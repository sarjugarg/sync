...
<body>
		...
		<i class="fa fa-external-link-square"></i> Schedule Campaign 
					...
					<form role="form"  id="formid"
										...
										<div class="form-group">
											...
											<!-- <input type="text" placeholder="Campaign Name"
													...
													<select class="form-control " name="channel" id='channelId' onchange="channelSelect();"  style="height: 30px; width:209px ;margin-top: 3px;" >
									 				<c:forEach items="${channel}" var="channel">
									 				<option value="${channel.channnelName }">${channel.channnelName }</option>
									 				</c:forEach>
											</select>
										...
										<div class="form-group">
											...
											<div class="col-sm-3">
												...
												<select class="form-control " disabled="disabled" name="campaign" id='campaignId' onchange="campaignSelect();" style="height: 30px; width:209px; margin-top: 3px;" >
								...
								<div class="col-sm-2 input-group date" id='startDate'>
									<input type='text' class="form-control" name="startDate" disabled="disabled"
										id="startDateI" value="" required /> <span class="input-group-addon"
										> <span
								...
								<div class="col-sm-2 input-group date" id='endDate'>
									<input type='text' class="form-control" name="endDate"  disabled="disabled"
										id='endDateI' value="" required /> <span class="input-group-addon"
										> <span
										...
										<div class="form-group">
											<label class="col-sm-2 control-label" for="form-field-1">
												Spot Count </label>
											<div class="col-sm-2">
												<input type="number" placeholder="spot count" id="spotId" disabled="disabled"
											onkeypress="spotSelect();"		class="form-control" name="spot count" required >
											</div>
										</div>
									<div class="form-group">
											<label class="col-sm-2 control-label" for="form-field-1">
												Cost </label>
											<div class="col-sm-2">
												<input type="number" placeholder="count" id="countId" disabled="disabled"
												onkeypress="countSelect();"	class="form-control" name="count" required>
											</div>
										</div>
										...
										<div class="form-group">
											<button type="button" class="btn btn-primary" onclick="uploadImage();" id="submitbtn"
												disabled="disabled" style="width: 10%; margin-right: 54%;">CREATE</button>
										</div>
										</div>
									</form>
</div>
...
function uploadImage(){
	...
	var resultArr = [];
	   ...
	   /*   formData.append('file', $('#audio_fileId')[0].files[0]);
	     ...
	     $.ajax({
			        	url: '${context}/addSheduleCampaign',
...
$(function() {
	...
	var now = new Date();
	var d = moment(now).format('YYYY-MM-DD hh:mm:ss');
	...
	$.ajax({
        ...
        success : function(data) {
        	...
        	for (i = 0; i < data.length; i++){
    			$('<option>').val(data[i]).channnelName.text(data[i]).channnelName.appendTo('#channelName');
